...
GameObject::GameObject(const Math::Vector2D &scene, const Math::Vector2D &position, const Graphics::Rect &bounds, bool visible): visible(visible), bounds(bounds), position(position), scene(scene){
}
